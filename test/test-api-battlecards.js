...
function crearPartida(id,nombre){
	...
	console.log("---------------------------------------------------------------------------");
	console.log("1. Intentar crear partida "+nombre);
	console.log("---------------------------------------------------------------------------");
	...
	request(options,function(error,response,body){
		if (!error && response.statusCode==200){
			//console.log(body);
			var jsonResponse = JSON.parse(body) ;
    		if (jsonResponse.partidaId!=-1){
    			id2=jsonResponse.usr;
	    		console.log("Partida id: "+jsonResponse.usr+" creada correctamente \n");
	    		elegirPartida(id2,"prueba");
	    	}
	    	else{
	    		console.log("No se pudo agregar al usuario \n");
	    	}
...
function elegirPartida(id,nombre){
	var options={
		url:url+'elegirPartida/'+id+'/'+nombre,
		method:'GET',
		headers:headers
	}

	console.log("---------------------------------------------------------------------------");
	console.log("1. Intentar elegir partida "+nombre);
	console.log("---------------------------------------------------------------------------");
	...
	request(options,function(error,response,body){
		if (!error && response.statusCode==200){
    		...
    		if (jsonResponse.partidaId!=-1){
    			id2=jsonResponse.usr;
	    		console.log("Partida id: "+jsonResponse.usr+" elegida correctamente \n");
	    		obtenerCartasMano(id1);
	    		obtenerCartasMano(id2);
	    	}
	    	else{
	    		console.log("No se pudo elegir partida \n");
	    	}
		}
		else{
			console.log(response.statusCode);
